	...
	return (
		<div className="relative pt-5">
			...
			>
				{options.map((option) => (
					<option
						className={`${styles[theme][2]} rounded-md py-2 px-4`}
						key={option.key}
					>
						{option.value}
					</option>
			...
			<LuChevronsUpDown className="absolute right-0 top-1/2 -translate-x-1/2 origin" />
			<label
				htmlFor={id}
				className={`top-4 -translate-y-6 peer-focus:start-0 peer-placeholder-shown:translate-y-0 peer-focus:-translate-y-6 absolute text-lg duration-300 transform scale-75 origin-[0] peer-focus:scale-75 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto ${styles[theme][1]}`}
			>
				{label}
			</label>
		</div>
